...
"""Ansible module for managing NVIDIA Bcm Node resources."""
...
---
module: bcm_node
short_description: Manage DGX/HGX nodes in BCM
description:
    - Register, update, and remove DGX/HGX nodes in Base Command Manager.
...
options:
    hostname:
        description:
            - The hostname or IP of the DGX/HGX node.
        type: str
        required: true
    cluster_id:
        description:
...
EXAMPLES = r"""
- name: Create a bcm node
  stevefulme1.nvidia_ai_factory.bcm_node:
    bcm_url: "https://bcm.example.com"
    bcm_token: "{{ bcm_token }}"
    state: present

- name: Delete a bcm node
  stevefulme1.nvidia_ai_factory.bcm_node:
    bcm_url: "https://bcm.example.com"
    bcm_token: "{{ bcm_token }}"
    node_id: "example-id"
...
def create_resource(module, client, base_url):
    ...
    resource_id = resource.get("id") or resource.get("node_id")
...
def needs_update(params, existing):
    ...
    desired = params.get("hostname")
    if desired is not None:
        current = existing.get("hostname")
        if current != desired:
            return True
    desired = params.get("node_type")
    if desired is not None:
        current = existing.get("node_type")
        if current != desired:
            return True
    desired = params.get("gpu_type")
    if desired is not None:
        current = existing.get("gpu_type")
        if current != desired:
            return True
    ...
    if desired is not None:
        ...
        if current != desired:
            return True
    desired = params.get("bmc_address")
    if desired is not None:
        current = existing.get("bmc_address")
        if current != desired:
            return True
    return False
...
def main():
    ...
    if params.get("node_id"):
        existing = get_resource(client, base_url, params["node_id"])
    else:
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, node=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, node=to_dict(existing))
