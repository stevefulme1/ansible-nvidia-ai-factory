...
"""Ansible module for querying NVIDIA Bcm Cluster resources."""
...
- name: List all resources
  stevefulme1.nvidia_ai_factory.bcm_cluster_info:
...
def main():
    ...
    if params.get("cluster_id"):
        url = f"{base_url}/api/v1/clusters/{params['cluster_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, clusters=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, clusters=[])
