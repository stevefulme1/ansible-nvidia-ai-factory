...
"""Ansible module for querying DGX power settings resources."""
...
DOCUMENTATION = r"""
---
module: dgx_power_info
short_description: List or get DGX power settings
description:
    - Retrieve information about DGX power settings.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.dgx_power_info:
...
def main():
    ...
    if params.get("power_id"):
        url = f"{base_url}/api/v1/dgx/power/{params['power_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, power_settings=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, power_settings=[])
            module.fail_json(msg=f"API error: {exc}")
    else:
        url = f"{base_url}/api/v1/dgx/power"
        query = {}
        if params.get("name"):
        ...
        try:
            ...
            module.exit_json(changed=False, power_settings=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
