...
"""Ansible module for querying Triton Inference Server instances resources."""
...
short_description: List or get Triton Inference Server instances
description:
...
def main():
    ...
    if params.get("server_id"):
        url = f"{base_url}/api/v1/triton/servers/{params['server_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, servers=[to_dict(resource)])
        ...
        try:
            ...
            module.exit_json(changed=False, servers=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
