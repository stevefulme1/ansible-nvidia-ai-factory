...
"""Ansible module for querying NVIDIA Nemo Model resources."""
...
DOCUMENTATION = r"""
---
module: nemo_model_info
short_description: List deployed NeMo models
description:
    - Retrieve information about NeMo model deployments.
...
- name: List all resources
  stevefulme1.nvidia_ai_factory.nemo_model_info:
...
def main():
    ...
    if params.get("model_id"):
        url = f"{base_url}/api/v1/models/{params['model_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, models=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, models=[])
