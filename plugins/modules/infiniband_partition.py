...
"""Ansible module for managing NVIDIA Infiniband Partition resources."""
...
options:
    ...
    tenant_id:
        description:
            - Associated tenant for isolation.
        type: str
    members:
        description:
...
def create_resource(module, client, base_url):
    ...
    resource_id = resource.get("id") or resource.get("partition_id")
...
def needs_update(params, existing):
    ...
    if desired is not None:
        ...
        if current != desired:
            return True
    desired = params.get("members")
    if desired is not None:
        current = existing.get("members")
        if current != desired:
            return True
    desired = params.get("qos_level")
    if desired is not None:
        current = existing.get("qos_level")
        if current != desired:
            return True
    return False
...
def main():
    ...
    existing = None
    if params.get("partition_id"):
        existing = get_resource(client, base_url, params["partition_id"])
    else:
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, ib_partition=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, ib_partition=to_dict(existing))
