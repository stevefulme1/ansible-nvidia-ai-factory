...
"""Ansible module for managing NVIDIA Triton Server resources."""
...
EXAMPLES = r"""
- name: Create a triton server
  stevefulme1.nvidia_ai_factory.triton_server:
    bcm_url: "https://bcm.example.com"
    bcm_token: "{{ bcm_token }}"
    state: present

- name: Delete a triton server
  stevefulme1.nvidia_ai_factory.triton_server:
    bcm_url: "https://bcm.example.com"
    bcm_token: "{{ bcm_token }}"
...
def get_module_args():
    module_args = dict(
        ...
        model_repository=dict(type="str", required=True),
        gpu_count=dict(type="int"),
        http_port=dict(type="int"),
        grpc_port=dict(type="int"),
        metrics_port=dict(type="int"),
...
def create_resource(module, client, base_url):
    ...
    resource_id = resource.get("id") or resource.get("server_id")
...
def needs_update(params, existing):
    ...
    if desired is not None:
        ...
        if current != desired:
            return True
    desired = params.get("http_port")
    if desired is not None:
        current = existing.get("http_port")
        if current != desired:
            return True
    desired = params.get("grpc_port")
    if desired is not None:
        current = existing.get("grpc_port")
        if current != desired:
            return True
    desired = params.get("metrics_port")
    if desired is not None:
        current = existing.get("metrics_port")
        if current != desired:
            return True
    return False
...
def main():
    ...
    if params.get("server_id"):
        existing = get_resource(client, base_url, params["server_id"])
    else:
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, triton_server=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, triton_server=to_dict(existing))
