...
"""Ansible module for querying NVIDIA Dgx Health resources."""
...
- name: List all resources
  stevefulme1.nvidia_ai_factory.dgx_health:
...
def main():
    ...
    if params.get("node_id"):
        url = f"{base_url}/redfish/v1/Systems/1/{params['node_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, health=[to_dict(resource)])
