...
"""Ansible module for querying DGX BIOS settings resources."""
...
DOCUMENTATION = r"""
---
module: dgx_bios_info
short_description: List or get DGX BIOS settings
description:
    - Retrieve information about DGX BIOS settings.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.dgx_bios_info:
...
def main():
    ...
    if params.get("bios_id"):
        url = f"{base_url}/api/v1/dgx/bios/{params['bios_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, bios_settings=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, bios_settings=[])
            module.fail_json(msg=f"API error: {exc}")
    else:
        url = f"{base_url}/api/v1/dgx/bios"
        query = {}
        if params.get("name"):
        ...
        try:
            ...
            module.exit_json(changed=False, bios_settings=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
