...
"""Ansible module for querying AMD GPU configurations resources."""
...
DOCUMENTATION = r"""
---
module: amd_gpu_config_info
short_description: List or get AMD GPU configurations
description:
    - Retrieve information about AMD GPU configurations.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.amd_gpu_config_info:
...
def main():
    ...
    if params.get("gpu_config_id"):
        url = f"{base_url}/api/v1/gpu/amd/configs/{params['gpu_config_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, gpu_configs=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, gpu_configs=[])
            module.fail_json(msg=f"API error: {exc}")
    else:
        url = f"{base_url}/api/v1/gpu/amd/configs"
        query = {}
        if params.get("name"):
        ...
        try:
            ...
            module.exit_json(changed=False, gpu_configs=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
