...
"""Ansible module for querying InfiniBand ports resources."""
...
DOCUMENTATION = r"""
---
module: infiniband_port_info
short_description: List or get InfiniBand ports
description:
    - Retrieve information about InfiniBand ports.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.infiniband_port_info:
...
def main():
    ...
    if params.get("port_id"):
        url = f"{base_url}/api/v1/infiniband/ports/{params['port_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, ports=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, ports=[])
        ...
        try:
            ...
            module.exit_json(changed=False, ports=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
