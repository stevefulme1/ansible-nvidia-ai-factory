...
"""Ansible module for querying NVIDIA Nvlink resources."""
...
- name: List all resources
  stevefulme1.nvidia_ai_factory.nvlink_info:
...
def main():
    ...
    if params.get("node_id"):
        url = f"{base_url}/api/v1/nvlink/{params['node_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, nvlink=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, nvlink=[])
