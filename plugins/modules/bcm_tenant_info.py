...
"""Ansible module for querying NVIDIA Bcm Tenant resources."""
...
- name: List all resources
  stevefulme1.nvidia_ai_factory.bcm_tenant_info:
...
def main():
    ...
    if params.get("tenant_id"):
        url = f"{base_url}/api/v1/tenants/{params['tenant_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, tenants=[to_dict(resource)])
