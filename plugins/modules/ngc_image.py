...
description:
    - Pull, list, and manage NVIDIA NGC container images.
...
def find_resource(client, base_url, params):
    ...
    name_val = params.get("image_name")
    if not name_val:
        return None
    ...
    try:
        ...
        for item in items:
            if item.get("state", "").upper() in DEAD_STATES:
                continue
            if item.get("image_name") == name_val:
                return item
...
def main():
    module = AnsibleModule(
        ...
        required_if=[("state", "present", ("image_name",), True)],
    )
