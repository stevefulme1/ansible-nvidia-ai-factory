...
"""Ansible module for querying NGC container images resources."""
...
short_description: List or get NGC container images
description:
    - Retrieve information about NGC container images.
...
def main():
    ...
    if params.get("image_name"):
        url = f"{base_url}/api/v1/ngc/images/{params['image_name']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, images=[to_dict(resource)])
        ...
        try:
            ...
            module.exit_json(changed=False, images=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
