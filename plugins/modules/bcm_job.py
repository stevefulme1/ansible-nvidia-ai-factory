...
"""Ansible module for managing NVIDIA Bcm Job resources."""
...
EXAMPLES = r"""
- name: Create a bcm job
  stevefulme1.nvidia_ai_factory.bcm_job:
    bcm_url: "https://bcm.example.com"
    bcm_token: "{{ bcm_token }}"
    state: present

- name: Delete a bcm job
  stevefulme1.nvidia_ai_factory.bcm_job:
    bcm_url: "https://bcm.example.com"
    bcm_token: "{{ bcm_token }}"
    job_id: "example-id"
...
def create_resource(module, client, base_url):
    ...
    resource_id = resource.get("id") or resource.get("job_id")
...
def main():
    ...
    if params.get("job_id"):
        existing = get_resource(client, base_url, params["job_id"])
    else:
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, job=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, job=to_dict(existing))
