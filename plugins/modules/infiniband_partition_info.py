...
"""Ansible module for querying InfiniBand partitions resources."""
...
DOCUMENTATION = r"""
---
module: infiniband_partition_info
short_description: List or get InfiniBand partitions
description:
    - Retrieve information about InfiniBand partitions.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.infiniband_partition_info:
...
def main():
    ...
    if params.get("partition_id"):
        url = f"{base_url}/api/v1/infiniband/partitions/{params['partition_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, partitions=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, partitions=[])
        ...
        try:
            ...
            module.exit_json(changed=False, partitions=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
