...
"""Ansible module for querying NVIDIA Infiniband resources."""
...
def main():
    ...
    if params.get("node_id"):
        url = f"{base_url}/api/v1/infiniband/fabric/{params['node_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, ib_fabric=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, ib_fabric=[])
            module.fail_json(msg=f"API error: {exc}")
    else:
        url = f"{base_url}/api/v1/infiniband/fabric"
        query = {}
