...
"""Ansible module for querying DGX health checks resources."""
...
DOCUMENTATION = r"""
---
module: dgx_health_info
short_description: List or get DGX health checks
description:
    - Retrieve information about DGX health checks.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.dgx_health_info:
...
def main():
    ...
    if params.get("health_id"):
        url = f"{base_url}/api/v1/dgx/health/{params['health_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, health_checks=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, health_checks=[])
            module.fail_json(msg=f"API error: {exc}")
    else:
        url = f"{base_url}/api/v1/dgx/health"
        query = {}
        if params.get("name"):
        ...
        try:
            ...
            module.exit_json(changed=False, health_checks=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
