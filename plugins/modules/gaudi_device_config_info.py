...
"""Ansible module for querying Gaudi device configurations resources."""
...
DOCUMENTATION = r"""
---
module: gaudi_device_config_info
short_description: List or get Gaudi device configurations
description:
    - Retrieve information about Gaudi device configurations.
...
- name: List all resources
  stevefulme1.gpu_ai_factory.gaudi_device_config_info:
...
def main():
    ...
    if params.get("device_id"):
        url = f"{base_url}/api/v1/gaudi/configs/{params['device_id']}"
        try:
            resp = call_with_retry(client.get, url, timeout=30)
            resp.raise_for_status()
            resource = resp.json()
            module.exit_json(changed=False, device_configs=[to_dict(resource)])
        except requests_lib.exceptions.HTTPError as exc:
            if exc.response is not None and exc.response.status_code == 404:
                module.exit_json(changed=False, device_configs=[])
            module.fail_json(msg=f"API error: {exc}")
    else:
        url = f"{base_url}/api/v1/gaudi/configs"
        query = {}
        if params.get("name"):
        ...
        try:
            ...
            module.exit_json(changed=False, device_configs=[to_dict(i) for i in items])
        except requests_lib.exceptions.HTTPError as exc:
            module.fail_json(msg=f"API error: {exc}")
