...
"""Unit tests for gaudi_device_config module."""
...
class TestMissingRequests:
    ...
    def test_missing_requests_fails(self, mock_module):
        set_module_args({"host": "https://gaudi.example.com", "api_key": "test-key", "device_id": "0", "state": "present"})

        with patch(
            "ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.gaudi_device_config.HAS_REQUESTS",
            False,
        ):
            from ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules import gaudi_device_config
            import importlib
            importlib.reload(gaudi_device_config)
            with patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.gaudi_device_config.HAS_REQUESTS", False):
                with pytest.raises(AnsibleFailJson) as exc_info:
                    gaudi_device_config.main()
...
class TestGet:
    ...
    @patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.gaudi_device_config.requests")
...
class TestCheckMode:
    ...
    def test_check_mode(self, mock_requests, mock_module):
        args = {"host": "https://gaudi.example.com", "api_key": "test-key", "device_id": "0", "state": "present"}
