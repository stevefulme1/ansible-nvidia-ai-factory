...
"""Unit tests for credential_store_info module."""
...
class TestMissingRequests:
    ...
    def test_missing_requests_fails(self, mock_module):
        set_module_args({"host": "https://vault.example.com", "api_key": "test-token"})

        with patch(
            "ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.credential_store_info.HAS_REQUESTS",
            False,
        ):
            from ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules import credential_store_info
            import importlib
            importlib.reload(credential_store_info)
            with patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.credential_store_info.HAS_REQUESTS", False):
                with pytest.raises(AnsibleFailJson) as exc_info:
                    credential_store_info.main()
...
class TestGet:
    ...
    @patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.credential_store_info.requests")
...
class TestCheckMode:
    ...
    def test_check_mode(self, mock_requests, mock_module):
        args = {"host": "https://vault.example.com", "api_key": "test-token"}
