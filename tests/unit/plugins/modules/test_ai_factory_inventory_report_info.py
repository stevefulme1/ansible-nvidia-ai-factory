...
"""Unit tests for ai_factory_inventory_report_info module."""
...
class TestMissingRequests:
    ...
    def test_missing_requests_fails(self, mock_module):
        set_module_args({"host": "https://aifactory.example.com", "api_key": "test-key"})

        with patch(
            "ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.ai_factory_inventory_report_info.HAS_REQUESTS",
            False,
        ):
            from ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules import ai_factory_inventory_report_info
            import importlib
            importlib.reload(ai_factory_inventory_report_info)
            with patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.ai_factory_inventory_report_info.HAS_REQUESTS", False):
                with pytest.raises(AnsibleFailJson) as exc_info:
                    ai_factory_inventory_report_info.main()
...
class TestGet:
    ...
    @patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.ai_factory_inventory_report_info.requests")
...
class TestCheckMode:
    ...
    def test_check_mode(self, mock_requests, mock_module):
        args = {"host": "https://aifactory.example.com", "api_key": "test-key"}
