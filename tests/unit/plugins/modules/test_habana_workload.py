...
"""Unit tests for habana_workload module."""
...
class TestMissingRequests:
    ...
    def test_missing_requests_fails(self, mock_module):
        set_module_args({"host": "https://gaudi.example.com", "api_key": "test-key", "name": "test-workload", "state": "present"})

        with patch(
            "ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.habana_workload.HAS_REQUESTS",
            False,
        ):
            from ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules import habana_workload
            import importlib
            importlib.reload(habana_workload)
            with patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.habana_workload.HAS_REQUESTS", False):
                with pytest.raises(AnsibleFailJson) as exc_info:
                    habana_workload.main()
...
class TestGet:
    ...
    @patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.habana_workload.requests")
...
class TestCheckMode:
    ...
    def test_check_mode(self, mock_requests, mock_module):
        args = {"host": "https://gaudi.example.com", "api_key": "test-key", "name": "test-workload", "state": "present"}
