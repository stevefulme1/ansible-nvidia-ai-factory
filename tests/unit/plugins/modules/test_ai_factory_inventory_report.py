...
"""Unit tests for ai_factory_inventory_report module."""
...
class TestMissingRequests:
    ...
    def test_missing_requests_fails(self, mock_module):
        set_module_args({"host": "https://aifactory.example.com", "api_key": "test-key", "state": "present"})

        with patch(
            "ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.ai_factory_inventory_report.HAS_REQUESTS",
            False,
        ):
            from ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules import ai_factory_inventory_report
            import importlib
            importlib.reload(ai_factory_inventory_report)
            with patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.ai_factory_inventory_report.HAS_REQUESTS", False):
                with pytest.raises(AnsibleFailJson) as exc_info:
                    ai_factory_inventory_report.main()
...
class TestGet:
    ...
    @patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.ai_factory_inventory_report.requests")
...
class TestCheckMode:
    ...
    def test_check_mode(self, mock_requests, mock_module):
        args = {"host": "https://aifactory.example.com", "api_key": "test-key", "state": "present"}
