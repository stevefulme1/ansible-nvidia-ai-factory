...
"""Unit tests for amd_rocm_driver module."""
...
class TestMissingRequests:
    ...
    def test_missing_requests_fails(self, mock_module):
        set_module_args({"host": "https://rocm.example.com", "api_key": "test-key", "state": "present"})

        with patch(
            "ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.amd_rocm_driver.HAS_REQUESTS",
            False,
        ):
            from ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules import amd_rocm_driver
            import importlib
            importlib.reload(amd_rocm_driver)
            with patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.amd_rocm_driver.HAS_REQUESTS", False):
                with pytest.raises(AnsibleFailJson) as exc_info:
                    amd_rocm_driver.main()
...
class TestGet:
    ...
    @patch("ansible_collections.stevefulme1.gpu_ai_factory.plugins.modules.amd_rocm_driver.requests")
...
class TestCheckMode:
    ...
    def test_check_mode(self, mock_requests, mock_module):
        args = {"host": "https://rocm.example.com", "api_key": "test-key", "state": "present"}
